...
const DashboardPrincipal = ({ isMobile }) => {
  ...
  // État initial avec les valeurs formatées
  ...
  const fetchPortfolioData = async () => {
    ...
    try {
      ...
      // Calcul et formatage des données
      const formattedData = {
        ...data,
  ...
  const renderPerformance = (performance) => {
    if (isLoading) return '...';
    return formatPercentage(performance);
  };

  const renderLastUpdate = (date) => {
    if (!date) return '';
      ...
      <div className="max-w-7xl mx-auto">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`font-bold ${isMobile ? 'text-xl' : 'text-2xl'}`}>
                    {renderValue(portfolioData.totalValue)}
                  </p>
                </div>
              </div>
            </CardContent>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`font-bold ${portfolioData.performance >= 0 ? 'text-green-500' : 'text-red-500'} ${isMobile ? 'text-xl' : 'text-2xl'}`}>
                    {renderPerformance(portfolioData.performance)}
                  </p>
                </div>
              </div>
            </CardContent>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">
                          {formatPercentage(portfolioData.totalValue > 0 ? 
                            (item.value / portfolioData.totalValue) * 100 : 0)} du portfolio
                        </p>
                      </div>
                    </div>
        ...
        {lastUpdate && (
          <div className={`text-white/60 text-sm text-center ${isMobile ? 'pb-4' : 'pb-6'}`}>
            Dernière mise à jour : {renderLastUpdate(lastUpdate)}
          </div>
        )}
      </div>
    </div>
